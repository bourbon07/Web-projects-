...
import { ShoppingCart, Heart, Search, ChefHat, Menu, X, Globe, User, LogOut, Settings, ClipboardList, ShieldCheck, Moon, Sun } from "lucide-react";
...
export function Header() {
  const {
    ...
    siteName, siteLogo, theme, setTheme
  } = useApp();
  const c = useEPTheme();
  ...
  // Initialize search from URL if on home
  useEffect(() => {
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/" className="flex items-center gap-2 shrink-0">
              <div
                ...
                style={{ background: siteLogo ? "transparent" : "linear-gradient(135deg, #E5233B, #FF6B8A)" }}
              >
                {siteLogo ? (
              ...
              <div className="flex items-center rounded-full border-2 overflow-hidden" style={{ borderColor: c.border, background: c.surface2 }}>
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (location.pathname === "/") {
                      navigate(`/?search=${encodeURIComponent(e.target.value)}`, { replace: true });
                    }
                  }}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button
                ...
                className="flex items-center justify-center p-2 rounded-full border transition-all hover:scale-110"
                style={{ borderColor: c.border, color: "#E5233B", background: c.surface }}
              >
              ...
              {isLoggedIn && currentUser?.isAdmin && (
                <Link
                  to="/admin"
                  className="hidden lg:flex items-center gap-1.5 px-4 py-1.5 rounded-full border-2 transition-all hover:scale-105"
                  style={{ borderColor: "#E5233B", color: "#E5233B", background: c.surface, fontWeight: 700 }}
                >
                  <ShieldCheck className="w-4 h-4" />
                  <span style={{ fontSize: "13px", fontFamily: "'Cairo', sans-serif" }}>
