...
export function Profile() {
  const { lang, currentUser, updateUserProfile, isLoggedIn } = useApp();
  const c = useEPTheme();
  ...
  // Sync form with currentUser whenever it changes (e.g., after save or login)
  useEffect(() => {
    if (currentUser) {
      setForm({
        name: currentUser.name || "",
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Parse Laravel 422 field-level validation errors
      const errors = error?.response?.data?.errors;
      if (errors?.phone) {
        // Show bilingual-friendly version of the phone error
        const msg = errors.phone[0];
        setPhoneError(
          lang === "ar"
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="rounded-2xl p-6 mb-5 space-y-4" style={{ background: c.surface, border: `1.5px solid ${c.border}` }}>
          ...
          <div>
            ...
            <div className="relative">
              <Phone className={`absolute top-3.5 ${lang === "ar" ? "right-3" : "left-3"} w-4 h-4`} style={{ color: phoneError ? "#EF4444" : "#E5233B" }} />
              <input
                type="tel"
                value={form.phone}
                ...
                style={{
                  ...
                  border: phoneError ? "1.5px solid #EF4444" : `1.5px solid ${c.border}`,
                }}
            ...
            {phoneError && (
              <p style={{ color: "#EF4444", fontSize: "12px", marginTop: "5px", fontWeight: 600, display: "flex", alignItems: "center", gap: "4px" }}>
                ⚠ {phoneError}
              </p>
            )}
          </div>
          ...
          {saveError && (
            <div className="p-3 rounded-xl flex items-center gap-2" style={{ background: "#FEF2F2", border: "1.5px solid #FECACA" }}>
              <span style={{ fontSize: "13px", color: "#EF4444", fontWeight: 600 }}>⚠ {saveError}</span>
            </div>
          )}
